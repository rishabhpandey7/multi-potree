...
export class Sidebar1{
	...
	init(){
		...
		this.dom.find('#potree_version_number').html(Potree.version.major + "." + Potree.version.minor + Potree.version.suffix);
	}
	...
	initToolbar(){
		...
		// ANGLE
		let elToolbar = this.dom.find('#tools');
		...
		{ // SHOW / HIDE Measurements
			let elShow = this.dom.find("#measurement_options_show");
	...
	initClippingTool(){
		...
		{
			let elClipTask = this.dom.find("#cliptask_options");
		...
		{
			let elClipMethod = this.dom.find("#clipmethod_options");
		...
		let clippingToolBar = this.dom.find("#clipping_tools");
	...
	initReturnFilters(){
		let elReturnFilterPanel = this.dom.find('#return_filter_panel');
	...
	initGPSTimeFilters(){
		...
		let elGPSTimeFilterPanel = this.dom.find('#gpstime_filter_panel');

		{
			...
			let initialize = () => {
				...
				let elRangeContainer = this.dom.find("#gpstime_multilevel_range_container");
	...
	initPointSourceIDFilters() {
		let elPointSourceIDFilterPanel = this.dom.find('#pointsourceid_filter_panel');
	...
	initClassificationList(){
		let elClassificationList = this.dom.find('#classificationList');
	...
	initAccordion(){
		this.dom.find('.accordion > h3').each(function(){
			let header = $(this);
		...
		let elLanguages = this.dom.find('#potree_languages');
	...
	initAppearance(){
		...
		this.dom.find('#sldEDLRadius').slider({
		...
		this.dom.find('#sldEDLStrength').slider({
		...
		this.dom.find('#sldEDLOpacity').slider({
		...
		this.viewer.addEventListener('fov_changed', (event) => {
			this.dom.find('#lblFOV')[0].innerHTML = parseInt(this.viewer.getFOV());
			this.dom.find('#sldFOV').slider({value: this.viewer.getFOV()});
		});

		this.viewer.addEventListener('use_edl_changed', (event) => {
			this.dom.find('#chkEDLEnabled')[0].checked = this.viewer.getEDLEnabled();
		});

		this.viewer.addEventListener('edl_radius_changed', (event) => {
			this.dom.find('#lblEDLRadius')[0].innerHTML = this.viewer.getEDLRadius().toFixed(1);
			this.dom.find('#sldEDLRadius').slider({value: this.viewer.getEDLRadius()});
		});

		this.viewer.addEventListener('edl_strength_changed', (event) => {
			this.dom.find('#lblEDLStrength')[0].innerHTML = this.viewer.getEDLStrength().toFixed(1);
			this.dom.find('#sldEDLStrength').slider({value: this.viewer.getEDLStrength()});
		});
		...
		{
			let elBackground = this.dom.find(`#background_options`);
			...
			this.dom.find(`input[name=background_options][value=${currentBackground}]`).trigger("click");
		}

		this.dom.find('#chkEDLEnabled').click( () => {
			this.viewer.setEDLEnabled(this.dom.find('#chkEDLEnabled').prop("checked"));
		});
	}

	initNavigation(){
		let elNavigation = this.dom.find('#navigation');
		let sldMoveSpeed = this.dom.find('#sldMoveSpeed');
		let lblMoveSpeed = this.dom.find('#lblMoveSpeed');
	...
	initSettings(){
		...
		{
			this.dom.find('#sldMinNodeSize').slider({
			...
			this.viewer.addEventListener('minnodesize_changed', (event) => {
				this.dom.find('#lblMinNodeSize').html(parseInt(this.viewer.getMinNodeSize()));
				this.dom.find('#sldMinNodeSize').slider({value: this.viewer.getMinNodeSize()});
			});
			this.dom.find('#lblMinNodeSize').html(parseInt(this.viewer.getMinNodeSize()));
		}

		{
			let elSplatQuality = this.dom.find("#splat_quality_options");
		...
		this.dom.find('#show_bounding_box').click(() => {
			this.viewer.setShowBoundingBox(this.dom.find('#show_bounding_box').prop("checked"));
		});

		this.dom.find('#set_freeze').click(() => {
			this.viewer.setFreeze(this.dom.find('#set_freeze').prop("checked"));
		});
	}
